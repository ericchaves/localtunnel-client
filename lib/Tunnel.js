...
class Tunnel extends EventEmitter {
  ...
  _getInfo(body) {
    ...
    const { dump_dir, local_reconnect, local_retry_max } = this.opts;
    return {
      name: id,
      url,
      cached_url,
  ...
  _establish(info) {
    ...
    this.tunnelCluster.on('error', err => {
      debug(
        'got socket error: %s [code=%s, state=%s, clientId=%s]',
        err.message,
        err.code || 'UNKNOWN',
    ...
    this.tunnelCluster.on('open', tunnel => {
      tunnelCount++;
      debug('tunnel open [total: %d/%d, clientId=%s]', tunnelCount, info.max_conn, this.clientId);
    ...
    this.tunnelCluster.on('dead', (eventInfo = {}) => {
      ...
      debug(
        'tunnel dead [total: %d, willReconnect=%s, closed=%s, retriable=%s]',
        tunnelCount,
        willReconnect,
        this.closed,
        retriable
      ...
      if (!retriable) {
        debug(
          'tunnel marked as non-retriable, not opening new connection [total: %d/%d]',
          tunnelCount,
          info.max_conn
        );
        ...
        if (tunnelCount === 0) {
          debug('all tunnels dead and non-retriable, closing client');
          this.close();
          process.exit(0);
        }
        return;
      ...
      if (tunnelCount < info.max_conn) {
        ...
        debug('reconnect delay increased to %dms (was %dms)', reconnectDelay, prevDelay);
      } else {
