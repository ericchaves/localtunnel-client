...
class TunnelCluster extends EventEmitter {
  constructor(opts = {}) {
    ...
    // Sequential tunnel ID for log identification
    this.nextTunnelId = 1;
  }

  /**
   * Generate log prefix for tunnel identification
   * Format: [clientId#tunnelId:port]
   * @param {number} tunnelId - The sequential tunnel ID
   * @param {net.Socket} socket - The socket to get port from (optional)
   * @returns {string} Log prefix like "[abc123#1:54321]" or "[abc123#1:?]"
   */
  _getTunnelLogPrefix(tunnelId, socket = null) {
    const port = socket?.localPort || '?';
    return `[${this.clientId}#${tunnelId}:${port}]`;
  }
  ...
  open() {
    ...
    debug(
      '%s establishing tunnel %s://%s:%s <> %s:%s',
      this._getTunnelLogPrefix(tunnelId),
      localProtocol,
      localHost,
      localPort,
      remoteHostOrIp,
    ...
    // Track socket for cleanup with tunnel ID
    this.sockets.push({ remote, local: null, deadEmitted: false, tunnelId });
    ...
    const connLocal = () => {
      if (this.closed || remote.destroyed) {
        debug(
          '%s remote destroyed or cluster closed (destroyed=%s, closed=%s)',
          this._getTunnelLogPrefix(tunnelId, remote),
          remote.destroyed,
          this.closed
        );
        if (!this.closed) {
      ...
      debug('%s connecting locally to %s://%s:%d', this._getTunnelLogPrefix(tunnelId, remote), localProtocol, localHost, localPort);
      remote.pause();

      if (allowInvalidCert) {
        debug('%s allowing invalid certificates (INSECURE - use only for development)', this._getTunnelLogPrefix(tunnelId, remote));
      }
      ...
      const remoteClose = () => {
        const socketPair = this.sockets.find(pair => pair.remote === remote);
        debug(
          '%s remote close (localHost=%s:%d, willEmitDead=%s, consecutive_failures=%d)',
          this._getTunnelLogPrefix(tunnelId, remote),
          localHost,
          localPort,
      ...
      local.once('error', err => {
        debug(
          '%s local error: %s [code=%s, localHost=%s:%d, consecutive_failures=%d]',
          this._getTunnelLogPrefix(tunnelId, remote),
          err.message,
          err.code,
          localHost,
        ...
        if (err.code === 'ECONNREFUSED' || err.code === 'ECONNRESET') {
          ...
          if (shouldGiveUp ||
            ...
            debug(
              '%s max consecutive failures reached [total=%d, refused=%d], marking tunnel as dead',
              this._getTunnelLogPrefix(tunnelId, remote),
              this.consecutiveLocalFailures,
              this.connectionRefusedCount
            );
          ...
          debug(
            '%s retrying local connection in %dms [attempt %d/%d]',
            this._getTunnelLogPrefix(tunnelId, remote),
          ...
          if (!this.closed && !remote.destroyed) {
            ...
            debug('%s local reconnect delay increased from %dms to %dms', this._getTunnelLogPrefix(tunnelId, remote), prevDelay, this.localReconnectDelay);
          }
        } else {
      ...
      local.once('connect', () => {
        debug('%s connected locally to %s://%s:%d', this._getTunnelLogPrefix(tunnelId, remote), localProtocol, localHost, localPort);
        ...
        if (this.consecutiveLocalFailures > 0 || this.connectionRefusedCount > 0 || this.connectionDroppedCount > 0) {
          debug(
            '%s resetting failure counters [total=%d, refused=%d, dropped=%d -> all 0]',
            this._getTunnelLogPrefix(tunnelId, remote),
            this.consecutiveLocalFailures,
            this.connectionRefusedCount,
        ...
        if (this.localReconnectDelay > 1000) {
          debug('%s resetting local reconnect delay [was=%dms, now=1000ms]', this._getTunnelLogPrefix(tunnelId, remote), this.localReconnectDelay);
        ...
        if (opt.local_host) {
          debug('%s transform Host header from server default to %s', this._getTunnelLogPrefix(tunnelId, remote), opt.local_host);
        ...
        local.once('close', hadError => {
          ...
          debug(
            '%s local closed [hadError=%s, localHost=%s:%d, consecutive_failures=%d, reconnect=%s]',
            this._getTunnelLogPrefix(tunnelId, remote),
            hadError,
            localHost,
            localPort,
          ...
          if (!shouldReconnect) {
            debug(
              '%s local reconnection disabled, closing remote tunnel [localHost=%s:%d]',
              this._getTunnelLogPrefix(tunnelId, remote),
              localHost,
              localPort
            );
          ...
          if (!hadError) {
            ...
            if (this.consecutiveLocalFailures > 0 || this.connectionRefusedCount > 0 || this.connectionDroppedCount > 0) {
              debug('%s resetting all failure counters after successful close', this._getTunnelLogPrefix(tunnelId, remote));
              this.consecutiveLocalFailures = 0;
              this.connectionRefusedCount = 0;
              this.connectionDroppedCount = 0;
            }
            ...
            if (!this.closed && !remote.destroyed) {
              debug('%s reconnecting local immediately for next request', this._getTunnelLogPrefix(tunnelId, remote));
            ...
            if (shouldGiveUp ||
              ...
              debug(
                '%s max consecutive failures on close [total=%d, dropped=%d], marking tunnel as dead',
                this._getTunnelLogPrefix(tunnelId, remote),
                this.consecutiveLocalFailures,
                this.connectionDroppedCount
              );
            ...
            if (!this.closed && !remote.destroyed) {
              debug(
                '%s retrying local after error close in %dms [attempt %d/%d]',
                this._getTunnelLogPrefix(tunnelId, remote),
                this.localReconnectDelay,
                this.consecutiveLocalFailures,
                maxConsecutiveFailures
    ...
    remote.on('data', data => {
      ...
      if (headerMatch) {
        debug('%s skipping server-originated message (X-LT-Source: server detected)', this._getTunnelLogPrefix(tunnelId, remote));
