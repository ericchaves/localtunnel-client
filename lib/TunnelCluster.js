    ...
    const connLocal = () => {
      ...
      local.once('connect', () => {
        ...
        local.once('close', hadError => {
          ...
          // Emit 'dead' directly since remoteClose listener may have been
          // removed by error handler (line 167) during a previous error
          const socketPair = this.sockets.find(pair => pair.remote === remote);
          if (socketPair && !socketPair.deadEmitted) {
            socketPair.deadEmitted = true;
            this.emit('dead');
          ...
          // Close remote connection for cleanup
          if (!remote.destroyed) {
            remote.end();
          }
        });
